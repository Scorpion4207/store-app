...
export const Navigation = () => {
  const { selectCart, selectToken } = useSelectHook();
      ...
      <ul className="flex gap-4">
        {selectToken && (
          <li className="flex">
            <Link
              to="/cart"
              className="flex flex-col items-center transition duration-300 ease-in-out hover:opacity-80"
            >
              <svg
              ...
              <span>{selectCart.length === 0 ? "корзина" : `${cartTotalPrice.toFixed(2)} $`}</span>
            </Link>
          </li>
        )}
        <li>
